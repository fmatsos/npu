...
Six [Agent Skills](https://code.claude.com/docs/en/skills): five that teach
Claude Code how to write and repair an `npu` configuration, one that cuts a
release of the repository itself. They are documentation Claude loads only
when it needs it — nothing runs at startup.
...
whichever one owns the file that failed. `release` is about the repository,
not about a configuration — it is the only one that pushes anything.
...
symlinks all six, so they load in every session here.
